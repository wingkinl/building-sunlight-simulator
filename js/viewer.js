...
(function () {
    ...
    const compassRotator = document.getElementById('uiCompassRotator');

    function updateCompassPointerRotation() {
        if (!compassRotator) return;
        const lookDir = new THREE.Vector3();
        lookDir.subVectors(controls.target, camera.position);
        lookDir.y = 0;
        if (lookDir.lengthSq() < 1e-9) return;
        lookDir.normalize();

        // In this project coordinate system, north points toward -Z.
        const heading = Math.atan2(lookDir.x, -lookDir.z);
        const pointerDeg = -heading * 180 / Math.PI;
        compassRotator.style.transform = `rotate(${pointerDeg.toFixed(2)}deg)`;
    }
